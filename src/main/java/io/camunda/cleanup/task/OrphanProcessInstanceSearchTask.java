...
public record OrphanProcessInstanceSearchTask(
    long parentProcessInstanceKey, Consumer<SearchRequestPage> page) implements Task {
  ...
  private void handleSearchResponse(
      SearchResponse<ProcessInstance> searchResponse, Consumer<Task> submitter) {
    if (!searchResponse.items().isEmpty()) {
      submitter.accept(
          new OrphanProcessInstanceSearchTask(
              parentProcessInstanceKey,
              p -> p.limit(100).after(searchResponse.page().endCursor())));
      searchResponse
          .items()
          .forEach(
              processInstance -> {
